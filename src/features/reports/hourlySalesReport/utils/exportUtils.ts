...
export const exportHourlySalesReportPDF = (
  ...
  const titleStr = `Hourly Sales Report - Branch: ${metadata.branchName}`;
  const dateStr = `From ${formatHeaderDate(metadata.fromDate)} To ${formatHeaderDate(metadata.toDate)}`;
  ...
  doc.save(`Hourly_Sales_Report_${metadata.fromDate}_${metadata.toDate}.pdf`);
};

export const exportHourlySalesReportExcel = (
  reportData: HourlySalesReportResponse["data"] | undefined,
  metadata: {
    branchName: string;
    fromDate: string;
  ...
  XLSX.writeFile(workbook, `Hourly_Sales_Report_${metadata.fromDate}_${metadata.toDate}.xlsx`);
};
