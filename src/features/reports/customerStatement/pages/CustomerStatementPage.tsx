...
import { exportCustomerStatementPDF, exportCustomerStatementExcel } from "../utils/customerStatementExportUtils";
...
import { CustomerStatementPrintPreviewModal } from "../components/CustomerStatementPrintPreviewModal";
...
const CustomerStatementPage = () => {
  ...
  const { filters, masterData, report } = useCustomerStatement();
  ...
  const branchOptions = useMemo(() => {
    return (masterData.branches as any[]).map((b: any) => ({
      label: b.branchName,
  ...
  const customerOptions = useMemo(() => [
    { label: "All", value: "0" },
    ...(masterData.customers as any[]).map((s: any) => ({
      label: s.code ? `[${s.code}] ${s.customerName}` : s.customerName,
      value: String(s.customerId)
    })),
  ...
  const handleExportPDF = useCallback(() => {
    exportCustomerStatementPDF(statementData, filters);
  }, [filters, statementData]);

  const handleExportExcel = useCallback(() => {
    exportCustomerStatementExcel(statementData, filters);
  }, [filters, statementData]);
  ...
  return (
    <PageShell title="Customer Statement">
      <div className="flex flex-col h-auto md:h-[calc(100vh-92px)] md:overflow-hidden p-1 gap-3 relative">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col flex-1 min-h-0">
          {!report.isLoading && statementData.length > 0 && (
            <p className="text-center text-[11px] font-semibold text-[#49293e] py-1.5 border-b border-gray-100 shrink-0">
              Customer Statement from {filters.fromDate} To {filters.toDate}
            </p>
          )}
        ...
        <CustomerStatementPrintPreviewModal
