...
interface CustomerStatementPrintPreviewModalProps {
  isOpen: boolean;
  onClose: () => void;
  data: PrintData;
  onExportPDF: () => void;
...
const CustomerStatementPrintTemplate = forwardRef<HTMLDivElement, { data: PrintData }>(
  ({ data }, ref) => {
    return (
      <div ref={ref} className="bg-white p-8 w-full max-w-[850px] shadow-sm font-sans text-slate-800 leading-relaxed mx-auto">
        <div className="header-container text-center mb-6">
          ...
          <div className="report-title text-xs font-bold underline text-black mt-2">
            Customer Statement From {formatHeaderDate(data.filters.fromDate)} To {formatHeaderDate(data.filters.toDate)}
          </div>
        </div>
...
CustomerStatementPrintTemplate.displayName = "CustomerStatementPrintTemplate";

export const CustomerStatementPrintPreviewModal = ({
  isOpen,
  onClose,
  data,
  onExportPDF,
}: CustomerStatementPrintPreviewModalProps) => {
...
<html>
  <head>
    <title>Customer Statement Print</title>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 flex justify-center bg-slate-200 rounded-b-xl min-h-0">
          <CustomerStatementPrintTemplate ref={printTemplateRef as any} data={data} />
        </div>
      </div>
    </div>
  );
