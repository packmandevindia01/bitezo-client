        ...
        <table className="w-full border-collapse mt-4 text-[10px]">
          <thead>
            <tr className="bg-[#49293e] text-white">
              ...
              <th className="text-left font-bold p-2 text-[10px] bg-[#49293e] text-white border-none w-[30%]">Voucher</th>
              <th className="text-left font-bold p-2 text-[10px] bg-[#49293e] text-white border-none w-[40%]">Particular</th>
              <th className="text-right font-bold p-2 text-[10px] bg-[#49293e] text-white border-none w-[20%]">Amount</th>
            </tr>
          </thead>
          <tbody>
            {data.reportData.length === 0 ? (
              ...
              data.reportData.map((row, idx) => (
                <tr key={idx} className={`border-b border-slate-100 ${idx % 2 === 1 ? "bg-slate-50/80" : "bg-white"}`}>
                  <td className="text-center p-2 text-[10px] text-slate-800 tabular-nums">{idx + 1}</td>
                  <td className="text-left p-2 text-[10px] text-slate-800 font-medium">{row.voucher || "-"}</td>
                  <td className="text-left p-2 text-[10px] text-slate-700">{row.particular || "-"}</td>
                  <td className="text-right p-2 text-[10px] text-slate-900 font-semibold tabular-nums">{formatAmount(Number(row.amount || 0))}</td>
                </tr>
              ))
            )}
            {/* Footer Summary Row */}
            {data.reportData.length > 0 && (
              <tr className="footer-row font-bold bg-white">
                <td colSpan={2} className="text-left p-2 text-[10px] text-slate-900 font-bold bg-white border-t border-slate-200">
                  {/* Left spacing */}
                </td>
