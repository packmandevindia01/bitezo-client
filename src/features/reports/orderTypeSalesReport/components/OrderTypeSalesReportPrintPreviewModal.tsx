...
interface OrderTypeSalesReportPrintPreviewModalProps {
  isOpen: boolean;
  onClose: () => void;
  onExportPDF: () => void;
  data: {
...
export const OrderTypeSalesReportPrintPreviewModal = ({
  isOpen,
  onClose,
  onExportPDF,
  data
}: OrderTypeSalesReportPrintPreviewModalProps) => {
        ...
        <div className="flex-1 overflow-auto bg-gray-100 rounded-lg p-2 flex justify-center border border-gray-200">
          ...
          >
            ...
            <div className="text-center mb-6">
              ...
              <h2 className="text-lg font-bold text-[#49293e] mt-4 uppercase">Order Type Sales Report</h2>
            </div>

            <div className="flex justify-between items-end mb-4 text-sm">
              <div>
                <p><span className="text-gray-500 w-16 inline-block">Period:</span> <span className="font-semibold text-gray-800">{formatDate(data.filters.fromDate)} To {formatDate(data.filters.toDate)}</span></p>
              ...
              <thead>
                <tr>
                  ...
                  {columns.map((col, idx) => (
                    <th 
                      key={idx} 
                      className={`px-3 py-2 border font-bold text-gray-900 bg-gray-50 ${col !== 'Date' ? 'text-right' : 'text-left'}`}
                    >
                      {col.toUpperCase()}
                    </th>
                  ))}
                ...
                {rows.map((row, idx) => (
                  <tr key={idx}>
                    ...
                    {columns.map((col, cIdx) => (
                      <td 
                        key={cIdx} 
                        className={`px-3 py-1.5 border text-gray-800 ${col !== 'Date' ? 'text-right font-medium' : 'text-left'}`}
                      >
                        {col === 'Date' ? (row[col] ? formatDate(String(row[col])) : "-") : formatAmount(Number(row[col]) || 0)}
                      </td>
                    ))}
                  </tr>
                ))}
