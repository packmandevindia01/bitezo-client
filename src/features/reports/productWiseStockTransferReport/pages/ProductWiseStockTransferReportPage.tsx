...
import { useProductWiseStockTransferReport } from "../hooks/useProductWiseStockTransferReport";
import {
  exportProductWiseStockTransferPDF,
  exportProductWiseStockTransferExcel,
...
import { ProductWiseStockTransferPrintPreviewModal } from "../components/ProductWiseStockTransferPrintPreviewModal";

// ─── Columns ──────────────────────────────────────────────────────────────────
const COLS = [
  { key: "sno",        label: "SNo",         cls: "w-[3%]  text-center" },
  { key: "date",       label: "Date",         cls: "w-[9%]  text-center" },
  { key: "refNo",      label: "Ref No",       cls: "w-[6%]  text-center" },
  { key: "fromBranch", label: "From Branch",  cls: "w-[13%] text-left"   },
  { key: "toBranch",   label: "To Branch",    cls: "w-[13%] text-left"   },
  { key: "code",       label: "Code",         cls: "w-[7%]  text-center" },
  { key: "product",    label: "Product",      cls: "w-[18%] text-left"   },
  { key: "qty",        label: "Qty",          cls: "w-[5%]  text-right"  },
...
const ProductWiseStockTransferReportPage = () => {
  const navigate = useNavigate();
  const { filters, masterData, report } = useProductWiseStockTransferReport();
  ...
  const handleExportPDF = useCallback(
    () => exportProductWiseStockTransferPDF(report.rows, report.grandTotals, filters),
    [report.rows, report.grandTotals, filters]
  );
  const handleExportExcel = useCallback(
    () => exportProductWiseStockTransferExcel(report.rows, report.grandTotals, filters),
    [report.rows, report.grandTotals, filters]
  );
  ...
  return (
    <PageShell title="Product Wise Stock Transfer Report">
      <div className="flex flex-col h-auto md:h-[calc(100vh-92px)] md:overflow-hidden p-1 gap-3 relative">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col flex-1 min-h-0">
          ...
          {!report.isLoading && report.rows.length > 0 && (
            <p className="text-center text-[11px] font-semibold text-[#49293e] py-1.5 border-b border-gray-100 shrink-0">
              Product Wise Stock Transfer from {filters.fromDate} To {filters.toDate}
            </p>
          )}
                ...
                {report.isLoading ? (
                  ...
                  report.rows.map((row, idx) => (
                    ...
                    >
                      ...
                      <td className="px-2 py-1.5 text-center font-mono text-gray-800 border-r border-gray-100">{row.refNo}</td>
                      <td className="px-2 py-1.5 text-left font-medium text-gray-800 border-r border-gray-100">{row.fromBranch}</td>
                      <td className="px-2 py-1.5 text-left font-medium text-gray-800 border-r border-gray-100">{row.toBranch}</td>
                      <td className="px-2 py-1.5 text-center text-gray-500 border-r border-gray-100">{row.code}</td>
                      <td className="px-2 py-1.5 text-left font-medium uppercase text-gray-800 border-r border-gray-100">{row.product}</td>
                      <td className="px-2 py-1.5 text-right tabular-nums text-gray-800 border-r border-gray-100">{row.qty}</td>
                      <td className="px-2 py-1.5 text-center text-gray-600 border-r border-gray-100">{row.unit}</td>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm px-4 py-2 shrink-0 flex items-center justify-between">
          <div className="flex items-center gap-5 text-xs font-medium text-gray-600">
            <span>
              Total Qty:{" "}
              <span className="font-bold text-gray-900 tabular-nums ml-1">{report.grandTotals.qty}</span>
            </span>
            <span>
              Total Amount:{" "}
        ...
        <ProductWiseStockTransferPrintPreviewModal
...
export default ProductWiseStockTransferReportPage;
