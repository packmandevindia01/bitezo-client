...
interface EmployeeSalesReportPrintPreviewModalProps {
  isOpen: boolean;
  onClose: () => void;
  data: PrintData;
  onExportPDF: () => void;
...
// ─── Print HTML Template ──────────────────────────────────────────────────────
const EmployeeSalesReportPrintTemplate = forwardRef<HTMLDivElement, { data: PrintData }>(
  ({ data }, ref) => {
    return (
      <div ref={ref} className="bg-white p-8 w-full max-w-[850px] shadow-sm font-sans text-slate-800 leading-relaxed mx-auto">
        ...
        <div className="header-container text-center mb-6">
          ...
          <div className="report-title text-xs font-bold underline text-black mt-2">
            Employee Sales Report From {formatHeaderDate(data.filters.fromDate)} To {formatHeaderDate(data.filters.toDate)}
          </div>
        </div>
        ...
        <table className="w-full border-collapse mt-4 text-[10px]">
          <thead>
            <tr className="bg-[#49293e] text-white">
              ...
              <th className="text-center font-bold p-2 text-[10px] bg-[#49293e] text-white border-none w-[15%]">Code</th>
              <th className="text-left font-bold p-2 text-[10px] bg-[#49293e] text-white border-none w-[35%]">Employee</th>
              <th className="text-right font-bold p-2 text-[10px] bg-[#49293e] text-white border-none w-[15%]">Net Value</th>
              <th className="text-right font-bold p-2 text-[10px] bg-[#49293e] text-white border-none w-[15%]">Vat Amount</th>
              <th className="text-right font-bold p-2 text-[10px] bg-[#49293e] text-white border-none w-[10%]">Amount</th>
            </tr>
          </thead>
          <tbody>
            {data.salesData.length === 0 ? (
              ...
              data.salesData.map((row, idx) => (
                <tr key={idx} className={`border-b border-slate-100 ${idx % 2 === 1 ? "bg-slate-50/80" : "bg-white"}`}>
                  <td className="text-center p-2 text-[10px] text-slate-800">{idx + 1}</td>
                  <td className="text-center p-2 text-[10px] text-slate-800 font-mono">{row.code || ""}</td>
                  <td className="text-left p-2 text-[10px] text-slate-800 font-medium uppercase">{row.employee || ""}</td>
...
EmployeeSalesReportPrintTemplate.displayName = "EmployeeSalesReportPrintTemplate";

// ─── Modal Container ──────────────────────────────────────────────────────────
export const EmployeeSalesReportPrintPreviewModal = ({
  isOpen,
  onClose,
  data,
  onExportPDF,
}: EmployeeSalesReportPrintPreviewModalProps) => {
...
<html>
  <head>
    <title>Employee Sales Report Print</title>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 flex justify-center bg-slate-200 rounded-b-xl min-h-0">
          <EmployeeSalesReportPrintTemplate ref={printTemplateRef as any} data={data} />
        </div>
      </div>
    </div>
  );
