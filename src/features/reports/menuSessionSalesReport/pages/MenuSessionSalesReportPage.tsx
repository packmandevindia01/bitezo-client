...
import { useMenuSessionSalesReport } from "../hooks/useMenuSessionSalesReport";
import {
  exportMenuSessionSalesReportPDF,
  exportMenuSessionSalesReportExcel,
...
import { MenuSessionSalesReportPrintPreviewModal } from "../components/MenuSessionSalesReportPrintPreviewModal";
...
export const MenuSessionSalesReportPage = () => {
  const navigate = useNavigate();
  const { filters, masterData, report } = useMenuSessionSalesReport();
  ...
  // Dynamic session column list (exclude Date)
  ...
  const handleExportPDF = useCallback(() => {
    exportMenuSessionSalesReportPDF(
      ...
      {
        branchName: filters.branchId === "0" ? "All" : filters.branchId,
  ...
  return (
    <PageShell title="Menu Session Sales Report">
                ...
                {report.isLoading ? (
                  <tr>
                    <td colSpan={dynamicPaymodes.length + 1} className="text-center py-12 text-gray-500">
                      Loading menu session sales data...
                    </td>
                  </tr>
                      ...
                      <tr key={rIdx} className="hover:bg-gray-50/70 transition-colors odd:bg-white even:bg-gray-50/20">
                        ...
                        {dynamicPaymodes.map((col) => {
                          ...
                          return (
                            <td
                              key={col}
                              className={`px-3 py-2 text-right border-r border-gray-100/60 font-mono ${
                                col === "Total"
                                  ? "font-bold text-[#49293e] bg-gray-50/50"
                                  : "text-gray-700"
                              }`}
                            >
                              {formatAmount(val)}
                  ...
                  <tr>
                    ...
                    {colTotals.map((tot, idx) => {
                      ...
                      return (
                        <td
                          key={idx}
                          className={`px-3 py-2 text-right border-r border-gray-200/60 font-mono ${
                            isTotalCol ? "text-[#49293e] bg-gray-50/70" : "text-gray-800"
                          }`}
                        >
                          {formatAmount(tot)}
                        </td>
      ...
      <MenuSessionSalesReportPrintPreviewModal
...
export default MenuSessionSalesReportPage;
