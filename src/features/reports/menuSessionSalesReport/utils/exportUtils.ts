...
import type { MenuSessionSalesReportResponse } from "../types";
...
export const exportMenuSessionSalesReportPDF = (
  reportData: MenuSessionSalesReportResponse["data"] | undefined,
  metadata: {
    branchName: string;
    fromDate: string;
    toDate: string;
  ...
  const titleStr = `Menu Session Sales Report - Branch: ${metadata.branchName}`;
  ...
  for (let i = 2; i <= reportData.columns.length; i++) {
    columnStyles[i] = { halign: "right" }; // All session monetary columns
  }
  ...
  doc.save(`MenuSession_Sales_Report_${metadata.fromDate}_${metadata.toDate}.pdf`);
};

export const exportMenuSessionSalesReportExcel = (
  reportData: MenuSessionSalesReportResponse["data"] | undefined,
  metadata: {
    branchName: string;
    fromDate: string;
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "Menu Session Sales Report");

  XLSX.writeFile(workbook, `MenuSession_Sales_Report_${metadata.fromDate}_${metadata.toDate}.xlsx`);
};
