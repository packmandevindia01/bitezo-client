...
import { usePurchaseReturnReport } from "../hooks/usePurchaseReturnReport";
import { exportPurchaseReturnReportPDF, exportPurchaseReturnReportExcel } from "../utils/purchaseReturnExportUtils";
...
const PurchaseReturnReportPage = () => {
  ...
  const groupedData = useMemo(() => {
    const rawData = report.purchaseReturnData;
    ...
    return rawData;
  }, [report.purchaseReturnData, groupBy]);

  const handleExportPDF = useCallback(() => {
    exportPurchaseReturnReportPDF(groupedData, report.paymodeData, report.totalData, {
      ...filters,
  ...
  }, [filters, branchOptions, supplierOptions, paymodeOptions, report, groupedData]);

  const handleExportExcel = useCallback(() => {
    exportPurchaseReturnReportExcel(groupedData, report.paymodeData, report.totalData, {
      ...filters,
      branchName:   branchOptions.find(o => o.value === filters.branchId)?.label,
      supplierName: supplierOptions.find(o => o.value === filters.supplierId)?.label,
      paymodeName:  paymodeOptions.find(o => o.value === filters.paymodeId)?.label,
  ...
  const grandTotals = useMemo(() => ({
    netValue:   groupedData.reduce((s, r) => s + Number(r.netValue  || 0), 0),
    vatAmount:  groupedData.reduce((s, r) => s + Number(r.vatAmount || 0), 0),
    netAmount:  groupedData.reduce((s, r) => s + Number(r.netAmount || 0), 0),
    billsundry: groupedData.reduce((s, r) => s + Number(r.billsundry || 0), 0),
    roundOff:   groupedData.reduce((s, r) => s + Number(r.roundOff   || 0), 0),
  }), [groupedData]);

  const cashTotal   = useMemo(() => report.purchaseReturnData.filter(r =>  r.paymode?.toLowerCase().includes("cash")).reduce((s, r) => s + Number(r.netAmount || 0), 0), [report.purchaseReturnData]);
  const creditTotal = useMemo(() => report.purchaseReturnData.filter(r => r.paymode && !r.paymode.toLowerCase().includes("cash")).reduce((s, r) => s + Number(r.netAmount || 0), 0), [report.purchaseReturnData]);

  const previewData = useMemo(() => ({
    purchaseReturnData: groupedData,
    cashTotal,
    creditTotal,
  ...
  }), [groupedData, cashTotal, creditTotal, grandTotals.netAmount, filters]);
  ...
  return (
    <PageShell title="Purchase Return Report">
      <div className="flex flex-col h-auto md:h-[calc(100vh-92px)] md:overflow-hidden p-1 gap-3 relative">
          ...
          <div className="flex items-center gap-3">
            {groupedData.length > 0 && (
              <span className="text-xs text-gray-400">
                {groupedData.length} record{groupedData.length !== 1 ? "s" : ""}
              </span>
            )}
            ...
            <Button
              ...
              disabled={report.isLoading || groupedData.length === 0}
            >
              Print Preview
            </Button>
            <Button
        ...
        <PurchaseReturnReportPrintPreviewModal
