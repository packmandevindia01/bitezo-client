...
export const exportPurchaseReturnReportPDF = (
  ...
  const dateStr = `Purchase Return Report From ${formatHeaderDate(filters.fromDate)} To ${formatHeaderDate(filters.toDate)}`;
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  ...
  // Table Data (Exact columns matching the prototype)
  const headers = [["BillDate", "BillNo", "Ref No", "Supplier", "Paymode", "Net Value", "Vat Amount", "Amount"]];
  const body = purchaseReturnData.map((row) => [
  ...
  const cashTotal = purchaseReturnData.filter(r => r.paymode?.toLowerCase().includes("cash")).reduce((s, r) => s + Number(r.netAmount || 0), 0);
  const creditTotal = purchaseReturnData.filter(r => r.paymode && !r.paymode.toLowerCase().includes("cash")).reduce((s, r) => s + Number(r.netAmount || 0), 0);
  const grandTotal = totalData ? totalData.netAmount : purchaseReturnData.reduce((s, r) => s + Number(r.netAmount || 0), 0);
...
export const exportPurchaseReturnReportExcel = (
  ...
  const totalNetValue = purchaseReturnData.reduce((s, r) => s + Number(r.netValue || 0), 0);
  const totalVatAmount = purchaseReturnData.reduce((s, r) => s + Number(r.vatAmount || 0), 0);
  const grandTotal = totalData ? totalData.netAmount : purchaseReturnData.reduce((s, r) => s + Number(r.netAmount || 0), 0);
  ...
  rows.push([cellHelper(`Purchase Return Report From ${formatHeaderDate(filters.fromDate)} To ${formatHeaderDate(filters.toDate)}`, true, "center", { font: { bold: true, sz: 11, underline: true } }), "", "", "", "", "", "", ""]);
  ...
  // 3. Table Data
  purchaseReturnData.forEach((row, idx) => {
    const dataStyle = {
      border: {
  ...
  XLSX.writeFile(wb, `Purchase_Return_Report_${filters.fromDate}_to_${filters.toDate}.xlsx`);
};
