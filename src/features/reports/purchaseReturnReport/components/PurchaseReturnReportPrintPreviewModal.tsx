...
interface PurchaseReturnReportPrintPreviewModalProps {
  isOpen: boolean;
  onClose: () => void;
  data: PrintData;
  onExportPDF: () => void;
...
// ─── Print HTML Template ──────────────────────────────────────────────────────
const PurchaseReturnReportPrintTemplate = forwardRef<HTMLDivElement, { data: PrintData }>(
  ({ data }, ref) => {
    return (
      <div ref={ref} className="bg-white p-8 w-full max-w-[850px] shadow-sm font-sans text-slate-800 leading-relaxed mx-auto">
        ...
        <div className="header-container text-center mb-6">
          ...
          <div className="report-title text-xs font-bold underline text-black mt-2">
            Purchase Return Report From {formatHeaderDate(data.filters.fromDate)} To {formatHeaderDate(data.filters.toDate)}
          </div>
        </div>
        ...
        <table className="w-full border-collapse mt-4 text-[10px]">
          ...
          <tbody>
            {data.purchaseReturnData.length === 0 ? (
              <tr>
              ...
              data.purchaseReturnData.map((row, idx) => (
...
PurchaseReturnReportPrintTemplate.displayName = "PurchaseReturnReportPrintTemplate";

// ─── Modal Container ──────────────────────────────────────────────────────────
export const PurchaseReturnReportPrintPreviewModal = ({
  isOpen,
  onClose,
  data,
  onExportPDF,
}: PurchaseReturnReportPrintPreviewModalProps) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 flex justify-center bg-slate-200 rounded-b-xl min-h-0">
          <PurchaseReturnReportPrintTemplate ref={printTemplateRef as any} data={data} />
        </div>
      </div>
    </div>
  );
