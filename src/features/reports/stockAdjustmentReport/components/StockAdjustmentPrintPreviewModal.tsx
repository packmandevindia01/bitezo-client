...
interface StockAdjustmentPrintPreviewModalProps {
  isOpen: boolean;
  onClose: () => void;
  data: PrintData;
  onExportPDF: () => void;
...
const StockAdjustmentPrintTemplate = forwardRef<HTMLDivElement, { data: PrintData }>(
  ({ data }, ref) => {
    return (
      <div ref={ref} className="bg-white p-8 w-full max-w-[850px] shadow-sm font-sans text-slate-800 leading-relaxed mx-auto">
        ...
        <div className="header-container text-center mb-6">
          ...
          <div className="report-title text-xs font-bold underline text-black mt-2">
            Stock Adjustment Report From {formatHeaderDate(data.filters.fromDate)} To {formatHeaderDate(data.filters.toDate)}
          </div>
        </div>
        ...
        <table className="w-full border-collapse mt-4 text-[10px]">
          <thead>
            <tr className="bg-[#49293e] text-white">
              <th className="text-center font-bold p-2 text-[10px] bg-[#49293e] text-white border-none w-[8%]">SNo</th>
              <th className="text-center font-bold p-2 text-[10px] bg-[#49293e] text-white border-none w-[18%]">Trans Date</th>
              <th className="text-center font-bold p-2 text-[10px] bg-[#49293e] text-white border-none w-[12%]">Ref No</th>
              <th className="text-left font-bold p-2 text-[10px] bg-[#49293e] text-white border-none w-[26%]">Branch</th>
              <th className="text-left font-bold p-2 text-[10px] bg-[#49293e] text-white border-none w-[24%]">Employee</th>
              <th className="text-right font-bold p-2 text-[10px] bg-[#49293e] text-white border-none w-[12%]">Net Amount</th>
            </tr>
          </thead>
          <tbody>
            {data.data.length === 0 ? (
              ...
              data.data.map((row, idx) => (
                <tr key={idx} className={`border-b border-slate-100 ${idx % 2 === 1 ? "bg-slate-50/80" : "bg-white"}`}>
                  ...
                  <td className="text-center p-2 text-[10px] text-slate-800 font-mono">{row.refNo || ""}</td>
                  <td className="text-left p-2 text-[10px] text-slate-800 font-medium">{row.branch || ""}</td>
                  <td className="text-left p-2 text-[10px] text-slate-800 font-medium">{row.employee || ""}</td>
            ...
            {data.data.length > 0 && (
              <tr className="footer-row font-bold bg-white">
                <td colSpan={4} className="text-left p-2 text-[10px] text-slate-900 font-bold bg-white border-t border-slate-200">
                ...
                <td className="text-right text-slate-900 text-[11px] font-bold tabular-nums p-2 bg-white border-t border-slate-200">
                  {formatAmount(data.totalNetAmount)}
                </td>
              </tr>
            )}
          </tbody>
...
StockAdjustmentPrintTemplate.displayName = "StockAdjustmentPrintTemplate";

export const StockAdjustmentPrintPreviewModal = ({
  isOpen,
  onClose,
  data,
  onExportPDF,
}: StockAdjustmentPrintPreviewModalProps) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 flex justify-center bg-slate-200 rounded-b-xl min-h-0">
          <StockAdjustmentPrintTemplate ref={printTemplateRef as any} data={data} />
        </div>
      </div>
    </div>
  );
