...
export const exportShiftEndReportPDF = (
  ...
  const head = [["SNo", "Start Date", "End Date", "User", "Counter", ...dynamicColumns]];
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      ...
      ...dynamicColumns.reduce((acc: any, _, idx) => {
        acc[idx + 5] = { halign: "right" }; // Index shift by 5 because of SNo, StartDate, EndDate, User, Counter
        return acc;
      }, {})
    },
    didParseCell: (data) => {
  ...
  doc.save(`Shift_End_Report_${filters.fromDate}_to_${filters.toDate}.pdf`);
};

export const exportShiftEndReportExcel = (
  dynamicColumns: string[],
  rows: Record<string, any>[],
  filters: any
) => {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "Shift End Report");

  XLSX.writeFile(
    workbook,
    `Shift_End_Report_${filters.fromDate}_to_${filters.toDate}.xlsx`
  );
};
