...
interface ShiftEndPrintPreviewModalProps {
  isOpen: boolean;
  onClose: () => void;
  data: PrintData;
  onExportPDF: () => void;
...
// ─── Print HTML Template ──────────────────────────────────────────────────────
const ShiftEndPrintTemplate = forwardRef<HTMLDivElement, { data: PrintData }>(
  ({ data }, ref) => {
    ...
    return (
      <div ref={ref} className="bg-white p-8 w-full max-w-[1100px] shadow-sm font-sans text-slate-800 leading-relaxed mx-auto">
        {/* Header */}
        <div className="header-container text-center mb-6">
          ...
          <div className="report-title text-xs font-bold underline text-black mt-2">
            Shift End Report From {data.filters.fromDate || "-"} To {data.filters.toDate || "-"}
          </div>
        </div>
        ...
        <table className="w-full border-collapse mt-4 text-[10px]">
          <thead>
            <tr className="bg-[#49293e] text-white">
              ...
              <th className="text-left font-bold p-2 text-[10px] bg-[#49293e] text-white border-none w-[12%]">Start Date</th>
              <th className="text-left font-bold p-2 text-[10px] bg-[#49293e] text-white border-none w-[12%]">End Date</th>
              <th className="text-left font-bold p-2 text-[10px] bg-[#49293e] text-white border-none w-[12%]">User</th>
              <th className="text-left font-bold p-2 text-[10px] bg-[#49293e] text-white border-none w-[12%]">Counter</th>
          ...
          <tbody>
            {data.rows.length === 0 ? (
              <tr>
                <td colSpan={data.dynamicColumns.length + 5} className="text-center py-10 text-slate-400">
                  No records found.
                </td>
              </tr>
            ) : (
              data.rows.map((row, idx) => (
                <tr key={idx} className={`border-b border-slate-100 ${idx % 2 === 1 ? "bg-slate-50/80" : "bg-white"}`}>
                  ...
                  <td className="text-left p-2 text-[10px] text-slate-800">{row.User || "-"}</td>
                  <td className="text-left p-2 text-[10px] text-slate-800">{row.Counter || "-"}</td>
                  {data.dynamicColumns.map((col) => {
...
ShiftEndPrintTemplate.displayName = "ShiftEndPrintTemplate";

// ─── Modal Container ──────────────────────────────────────────────────────────
export const ShiftEndPrintPreviewModal = ({
  isOpen,
  onClose,
  data,
  onExportPDF,
}: ShiftEndPrintPreviewModalProps) => {
...
<html>
  <head>
    <title>Shift End Report Print</title>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 flex justify-center bg-slate-200 rounded-b-xl min-h-0">
          <ShiftEndPrintTemplate ref={printTemplateRef as any} data={data} />
        </div>
      </div>
    </div>
  );
