...
import { useDayEndReport } from "../hooks/useDayEndReport";
import {
  exportDayEndReportPDF,
  exportDayEndReportExcel,
...
import { DayEndPrintPreviewModal } from "../components/DayEndPrintPreviewModal";
...
export const DayEndReportPage = () => {
  const navigate = useNavigate();
  const { filters, masterData, report } = useDayEndReport();
  ...
  const excludedColumns = ["DayId", "SNo", "StartDate", "EndDate"];
  ...
  const handleExportPDF = useCallback(() => {
    exportDayEndReportPDF(dynamicColumns, report.rows, {
      ...filters,
  ...
  const handleExportExcel = useCallback(() => {
    exportDayEndReportExcel(dynamicColumns, report.rows, {
      ...filters,
      branchName: branchOptions.find(o => o.value === filters.branchId)?.label,
    });
  }, [report, filters, branchOptions]);
  ...
  return (
    <PageShell title="Day End Report">
            ...
            <table className="w-full min-w-[900px] text-xs border-collapse table-fixed">
              <thead className="sticky top-0 z-10 bg-gray-100 shadow-[inset_0_-1px_0_rgba(0,0,0,0.06)]">
                <tr>
                  ...
                  <th className="px-3 py-2 font-bold text-gray-600 text-[11px] text-left w-[15%] border-r border-gray-200/60">
                    Start Date
                  </th>
                  <th className="px-3 py-2 font-bold text-gray-600 text-[11px] text-left w-[15%] border-r border-gray-200/60">
                    End Date
                  ...
                  {dynamicColumns.map((col) => (
                    <th
                      key={col}
                      className={`px-3 py-2 font-bold text-gray-600 text-[11px] text-right border-r border-gray-200/60 ${col === 'Total' ? 'bg-gray-200/50 w-[12%]' : ''}`}
                    >
                      {col}
                    </th>
                  ))}
              ...
              <tbody className="divide-y divide-gray-100">
                {report.isLoading ? (
                  <tr>
                    <td colSpan={dynamicColumns.length + 3} className="text-center py-12 text-gray-500">
                      Loading day end data...
                    </td>
                  </tr>
                ) : report.rows.length === 0 ? (
                  <tr>
                    <td colSpan={dynamicColumns.length + 3} className="text-center py-12 text-gray-400 font-medium">
                      No records found.
                    </td>
                  </tr>
                ) : (
