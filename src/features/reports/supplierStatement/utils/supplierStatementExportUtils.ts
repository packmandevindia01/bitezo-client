...
export const exportSupplierStatementPDF = async (
  ...
  const dateStr = `Supplier Statement From ${formatHeaderDate(filters.fromDate)} To ${formatHeaderDate(filters.toDate)}`;
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
...
export const exportSupplierStatementExcel = async (
  statementData: SupplierStatementData[],
  filters: any
) => {
  ...
  rows.push([cellHelper(`Supplier Statement From ${formatHeaderDate(filters.fromDate)} To ${formatHeaderDate(filters.toDate)}`, true, "center", { font: { bold: true, sz: 11, underline: true } }), "", "", "", ""]);
  rows.push(["", "", "", "", ""]);
  ...
  XLSX.writeFile(wb, `Supplier_Statement_${filters.fromDate}_to_${filters.toDate}.xlsx`);
};
