...
export const stockAdjustmentApi = {
  ...
  getPurchaseCostData: async (barcode: string) => {
    const response = await axiosInstance.get<ApiResponse<{ productId: number; productCode: string; productName: string; baseUnitId: number; cost: number; altUnitId: number; vatId: number; vatName: string; vatValue: number }>>(`${BASE_URL}/purchase-cost-data/${barcode}`);
    return unwrap(response.data);
  },
