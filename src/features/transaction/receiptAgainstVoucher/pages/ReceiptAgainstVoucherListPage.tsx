...
const ReceiptAgainstVoucherListPage = () => {
  ...
  const { records, isLoading, branches, searchBranchId, setSearchBranchId, isBranchLocked } = useReceiptAgainstVoucherList(fromDate, toDate);
  ...
  const deleteMutation = useMutation({
    mutationFn: receiptAgainstVoucherApi.deleteReceiptAgainstVoucher,
    onSuccess: () => {
      showToast("Receipt against voucher cancelled successfully", "success", "Success");
      queryClient.invalidateQueries({ queryKey: ["receiptAgainstVoucherList"] });
      setDeleteId(null);
    },
    onError: (error: any) => {
  ...
  return (
    <PageShell title="Receipt Against Vouchers">
      <div className="flex flex-col md:flex-row gap-4 mb-4 justify-between items-end">
        ...
        <Button 
          icon={<Plus size={18} />}
          onClick={() => navigate("/dashboard/receipt-against-voucher/new")}
        >
          Add New
        </Button>
      </div>
      ...
      <ConfirmDialog
        ...
        message="Are you sure you want to cancel this receipt against voucher? This action cannot be undone."
...
export default ReceiptAgainstVoucherListPage;
