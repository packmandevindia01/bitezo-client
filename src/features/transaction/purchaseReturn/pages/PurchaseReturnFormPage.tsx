...
const PurchaseReturnFormPage = () => {
  ...
  const canAdd = hasPermission("Purchase Return", "Add");
  const canEdit = hasPermission("Purchase Return", "Edit");
    ...
    <PageShell title="Purchase Return">
      ...
      <div className="rounded-3xl border border-gray-200 bg-white shadow-sm flex flex-col" style={{ maxHeight: "calc(100vh - 120px)" }}>
        ...
        <div className="grid gap-x-3 gap-y-2 md:grid-cols-5 xl:grid-cols-9">
          ...
          <FormInput id="pr-invoiceNo" label="Inv No" value={form.invoiceNo} onChange={(e) => setField("invoiceNo", e.target.value)} onKeyDown={(e) => hk(e, "pr-refNo")} required readOnly={!canSave} />
        ...
        <div className="mt-1 rounded-xl border border-gray-200 bg-gray-50/70 p-2">
          <div className="grid gap-x-2 gap-y-1 md:grid-cols-[1.5fr_1fr_1fr_0.6fr_0.6fr_0.8fr_0.7fr_0.7fr_0.8fr_0.8fr_0.8fr_0.9fr_auto]">
            <SearchableSelect
              ...
              onKeyDown={(e) => hk(e, "pr-code")}
              disabled={!canSave}
            />
            <FormInput id="pr-code" label="Code" value={form.code} onChange={(e) => setField("code", e.target.value)} onKeyDown={(e) => hk(e, "pr-unit")} readOnly={!canSave} />
            <FormInput id="pr-unit" label="Unit" value={form.unit} onChange={(e) => setField("unit", e.target.value)} onKeyDown={(e) => hk(e, "pr-qty")} readOnly={!canSave} />
            <FormInput id="pr-qty" label="Qty" value={form.qty} inputClassName="text-right" onChange={(e) => setField("qty", e.target.value)} onKeyDown={(e) => hk(e, "pr-foc")} readOnly={!canSave} />
            <FormInput id="pr-foc" label="FOC" value={form.foc} inputClassName="text-right" onChange={(e) => setField("foc", e.target.value)} onKeyDown={(e) => hk(e, "pr-price")} readOnly={!canSave} />
            <FormInput id="pr-price" label="Price" value={form.price} inputClassName="text-right" onChange={(e) => setField("price", e.target.value)} onKeyDown={(e) => hk(e, "pr-vatPercent")} readOnly={!canSave} />
            <FormInput id="pr-vatPercent" label="VAT(%)" value={form.vatPercent} inputClassName="text-right" onChange={(e) => setField("vatPercent", e.target.value)} onKeyDown={(e) => hk(e, "pr-discPercent")} readOnly={!canSave} />
            <FormInput id="pr-discPercent" label="Disc(%)" value={form.discPercent} inputClassName="text-right" onChange={(e) => setField("discPercent", e.target.value)} onKeyDown={(e) => hk(e, "pr-add-btn")} readOnly={!canSave} />
