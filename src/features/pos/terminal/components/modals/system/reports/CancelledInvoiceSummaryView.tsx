...
import type { VoidInvoiceSummaryItem } from './types';

interface CancelledInvoiceSummaryViewProps {
  fromDate: string;
  toDate: string;
  fromTime?: string;
  toTime?: string;
  ...
  loading: boolean;
  logs: VoidInvoiceSummaryItem[];
  onPrint: (directPrint: boolean) => void;
  formatDate: (dateStr: string) => string;
}

export const CancelledInvoiceSummaryView: React.FC<CancelledInvoiceSummaryViewProps> = ({
  fromDate,
  toDate,
  fromTime = "00:00",
  toTime = "23:59",
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center justify-between bg-slate-50 p-3.5 rounded-2xl border border-slate-200/80 gap-3">
        <div>
          ...
          <p className="text-[11px] text-slate-500 font-medium">
            Log of cancelled/voided customer invoices, bill #, order # & amount
          </p>
        </div>
      ...
      <div className="border border-slate-200/80 rounded-2xl overflow-hidden bg-white shadow-xs min-h-[300px] flex flex-col justify-between">
              ...
              {loading ? (
                ...
                logs.map((row) => (
                  <tr key={row.sNo} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-2.5 px-4 text-xs font-black text-purple-700 border-r border-slate-100 text-center">{row.billNo}</td>
                    ...
                    <td className="py-2.5 px-4 text-xs font-black text-purple-600 text-right">{formatCurrency(row.amount)}</td>
                  </tr>
                ))
              )}
            </tbody>
        ...
        <div className="bg-slate-100 px-6 py-3 border-t border-slate-200 flex items-center justify-between font-black text-xs text-[#49293e]">
          <span>TOTAL CANCELLED INVOICES: {logs.length}</span>
