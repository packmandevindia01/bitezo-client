...
import type { AllTransactionSummaryItem } from './types';

interface AllTransactionSummaryViewProps {
  fromDate: string;
  toDate: string;
  fromTime?: string;
  toTime?: string;
  ...
  loading: boolean;
  logs: AllTransactionSummaryItem[];
  onPrint: (directPrint: boolean) => void;
  formatDate: (dateStr: string) => string;
}

export const AllTransactionSummaryView: React.FC<AllTransactionSummaryViewProps> = ({
  fromDate,
  toDate,
  fromTime = "00:00",
  toTime = "23:59",
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center justify-between bg-slate-50 p-3.5 rounded-2xl border border-slate-200/80 gap-3">
        <div>
          ...
          <p className="text-[11px] text-slate-500 font-medium">
            Summary of cash, credit, discounts, pending orders & transactions
          </p>
        </div>
      ...
      <div className="border border-slate-200/80 rounded-2xl overflow-hidden bg-white shadow-xs min-h-[300px] flex flex-col justify-between">
          ...
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 bg-slate-100 z-10 border-b border-slate-200">
              <tr>
                ...
                <th className="py-2.5 px-4 text-[10px] font-extrabold text-slate-600 uppercase tracking-wider border-r border-slate-200">Particulars</th>
              ...
              {loading ? (
                <tr>
                  <td colSpan={3} className="py-16 text-center">
                    <Loader text="Loading transaction summary..." />
                  </td>
                </tr>
              ) : logs.length === 0 ? (
                <tr>
                  <td colSpan={3} className="py-16 text-center text-xs font-bold text-slate-400 uppercase tracking-widest">
                    No transaction summary records found
                  </td>
                </tr>
              ) : (
                logs.map((row, index) => (
                  <tr key={index} className="hover:bg-slate-50/80 transition-colors">
                    <td className="py-2.5 px-4 text-xs font-bold text-slate-600 border-r border-slate-100 text-center">{index + 1}</td>
                    <td className="py-2.5 px-4 text-xs font-black text-indigo-900 border-r border-slate-100">{row.particular || '—'}</td>
                    <td className="py-2.5 px-4 text-xs font-black text-indigo-700 text-right">{formatCurrency(row.amount)}</td>
                  </tr>
                ))
              )}
            </tbody>
        ...
        <div className="bg-slate-100 px-6 py-3 border-t border-slate-200 flex items-center justify-between font-black text-xs text-[#49293e]">
          <span>TOTAL PARTICULARS: {logs.length}</span>
