...
import type { VoidProductSummaryItem } from './types';

interface VoidProductSummaryViewProps {
  fromDate: string;
  toDate: string;
  fromTime?: string;
  toTime?: string;
  ...
  loading: boolean;
  logs: VoidProductSummaryItem[];
  onPrint: (directPrint: boolean) => void;
  formatDate: (dateStr: string) => string;
}

export const VoidProductSummaryView: React.FC<VoidProductSummaryViewProps> = ({
  fromDate,
  toDate,
  fromTime = "00:00",
  toTime = "23:59",
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center justify-between bg-slate-50 p-3.5 rounded-2xl border border-slate-200/80 gap-3">
        <div>
          ...
          <p className="text-[11px] text-slate-500 font-medium">
            Log of voided items, order #, employee details, quantities & amount
          </p>
        </div>
      ...
      <div className="border border-slate-200/80 rounded-2xl overflow-hidden bg-white shadow-xs min-h-[300px] flex flex-col justify-between">
          ...
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 bg-slate-100 z-10 border-b border-slate-200">
              <tr>
                ...
                <th className="py-2.5 px-4 text-[10px] font-extrabold text-slate-600 uppercase tracking-wider border-r border-slate-200">Void Date</th>
                ...
                <th className="py-2.5 px-4 text-[10px] font-extrabold text-slate-600 uppercase tracking-wider border-r border-slate-200">Product</th>
                <th className="py-2.5 px-4 text-[10px] font-extrabold text-slate-600 uppercase tracking-wider border-r border-slate-200 text-center">Qty</th>
              ...
              {loading ? (
                <tr>
                  <td colSpan={8} className="py-16 text-center">
                    <Loader text="Loading void product summary..." />
                  </td>
                </tr>
              ) : logs.length === 0 ? (
                <tr>
                  <td colSpan={8} className="py-16 text-center text-xs font-bold text-slate-400 uppercase tracking-widest">
                    No voided product records found
                  </td>
                </tr>
              ) : (
                logs.map((row) => (
                  <tr key={row.sNo} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-2.5 px-4 text-xs text-slate-600 border-r border-slate-100">{formatDate(row.voidDate)}</td>
                    <td className="py-2.5 px-4 text-xs font-bold text-slate-700 border-r border-slate-100">{row.employee}</td>
                    <td className="py-2.5 px-4 text-xs font-black text-rose-700 border-r border-slate-100">{row.product}</td>
                    <td className="py-2.5 px-4 text-xs font-extrabold text-slate-800 border-r border-slate-100 text-center">{row.quantity} {row.unit}</td>
                    <td className="py-2.5 px-4 text-xs font-black text-rose-600 text-right">{formatCurrency(row.amount)}</td>
                  </tr>
                ))
              )}
            </tbody>
        ...
        <div className="bg-slate-100 px-6 py-3 border-t border-slate-200 flex items-center justify-between font-black text-xs text-[#49293e]">
          <span>TOTAL ITEMS: {logs.length}</span>
          <div className="flex items-center gap-6 text-xs">
            <div>
              <span className="text-slate-500 mr-2">TOTAL QTY:</span>
              <span className="text-slate-900 font-extrabold">{totalQty}</span>
            </div>
            <div>
              <span className="text-slate-500 mr-2">TOTAL VOID AMOUNT:</span>
              <span className="text-rose-600 font-black">{formatCurrency(totalAmount)}</span>
            </div>
          </div>
        </div>
      </div>
